...
root_dir = "../"
default_input_dir = root_dir + "input_data_1"
...
default_shared_dir = root_dir + "sample_code_submission"
# =============================================================================
# =========================== END USER OPTIONS ================================
# =============================================================================
...
if __name__=="__main__":	
    ...
    if len(argv)==1: # Use the default input and output directories if no arguments are provided
        ...
        hidden_dir = os.path.abspath(argv[5])
        shared_dir = default_shared_dir
    if verbose: 
    ...
    import sys
    sys.path.insert(0, default_output_dir )
    ...
    score_file = open(os.path.join(output_dir, 'scores1.txt'), 'w')
    html_file = open(os.path.join(output_dir, 'scores1.html'), 'w')
    ...
    dataname = os.listdir(input_dir)

    input_folder_name = ""
    for i in os.listdir(input_dir):
        score_file.write(str(i) + '\n')
        input_folder_name = os.path.join(input_dir, i)
    score_file.write(str(input_folder_name) + '\n')

    hidden_folder_name = ""
    for i in os.listdir(hidden_dir):
        score_file.write(str(i) + '\n')
        hidden_folder_name = os.path.join(hidden_dir, i)
    score_file.write(str(hidden_folder_name) + '\n')

    datanames = []
    for i in dataname:
        for j in os.listdir(os.path.join(input_dir,i)):
            datanames.append(os.path.join(j))
    score_file.write(str(datanames) + '\n')
    ...
    X_test = []
    for index_basename, basename in enumerate(datanames): # We have only one training dataset for the moment
        score_file.write('    ' + str(basename) + '\n')
        X_train = []
        vprint( verbose,  "\n========== Ingestion program version " + str(version) + " ==========\n") 
        vprint( verbose,  "************************************************")
        ...
        dataset_names = os.listdir(os.path.join(input_folder_name,basename))
        for i in dataset_names:
            score_file.write('        ' + str(i) + '\n')
            x = np.load(input_folder_name +'/'+ basename +'/'+ i)
            y = np.load(hidden_folder_name +'/'+ basename + '/'+ i)
            X_train.append(x)
            Y_train.append(y)

        # ======== Keeping track of time
        if debug_mode<1:
            time_budget = 600000        # <== HERE IS THE TIME BUDGET!
        else:
        ...
        if basename == 'dev':
            modelname = os.path.join(submission_dir +'/',datanames[index_basename-1])
        print(modelname)
        score_file.write(str(modelname) + '\n')
        ...
        if you_must_train:
            ...
            if save_model:
                ...
                vprint( verbose, "======== Saving model to: " + submission_dir)
                print(outname)
                M.save(outname)
        ...
        if True:
            data_io.mkdir(output_dir +'/'+ basename)
            dataset_names = os.listdir(os.path.join(input_folder_name,basename))
            for i in range(len(dataset_names)):
                np.save(output_dir +'/'+ basename + '/'+ dataset_names[i], np.array(y_train[i]))
