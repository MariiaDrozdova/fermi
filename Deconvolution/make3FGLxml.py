#!/usr/bin/env python
#updated 26/05/2017 with fixAll=False
#updated 15/10/2017 with iso/galactic normalization 0-100
...
class srcList:
	...
	def makeModel(self,GDfile="$(FERMI_DIR)/refdata/fermi/galdiffuse/gll_iem_v06.fits",GDname='gll_iem_v06',ISOfile="$(FERMI_DIR)/refdata/fermi/galdiffuse/iso_P8R2_SOURCE_V6_v06.txt",ISOname='iso_P8R2_SOURCE_V6_v06',normsOnly=True,extDir='Templates',radLim=-1,maxRad=None,ExtraRad=0,sigFree=5,varFree=True,psForce=False,makeRegion=True,GIndexFree=False,ApplyEDisp=True,wd='',oldNames=False):
...
def addSrcsXML(sL,GD,GDn,ISO,ISOn,oldNames=False):
	...
	for src in catalog:
		...
		if dist<=sL.roi[2]+sL.ER:
			...
			fixAll=False #(True if str(sname) in ['3FGL J0534.5+2201i','3FGL J0833.1-4511e','3FGL J1514.0-5915e','3FGL J2021.0+4031e','3FGL J2028.6+4110e'] else False)#account for sources held fixed in 3FGL analysis
...
def addSrcsFITS(sL,GD,GDn,ISO,ISOn,oldNames):
			...
			if (dist<x and dist>=x-step) or (x==sL.roi[2]+10. and dist==x):
				...
				if E and not sL.psF:
					...
					for EXTNAME,EXTFILE in zip(extName,extFile):
						if En.strip()==EXTNAME.strip():
							efile=sL.extD+EXTFILE.strip()
					if efile==None:
	...
	spec+='\t\t<parameter free="1" max="100" min="0" name="Prefactor" scale="1" value="1"/>\n'
	if sL.GIF:
	...
	spec+='\t\t<parameter free="1" max="100" min="0" name="Normalization" scale="1" value="1"/>\n'
	spec+='\t</spectrum>\n'
